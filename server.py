...
# CUDA optimizations for maximum speed
if torch.cuda.is_available():
    ...
    logging.info("CUDA optimizations enabled: cuDNN benchmark, TF32, Flash SDP")
...
def get_base_model():
    ...
    logging.info(f"Loading Base model: {BASE_MODEL_NAME} with FlashAttention2...")
...
def get_voice_design_model():
    ...
    logging.info(f"Loading VoiceDesign model: {VOICE_DESIGN_MODEL_NAME} with FlashAttention2...")
    model = Qwen3TTSModel.from_pretrained(
...
# Use 'tiny' for faster transcription (only used for reference audio, not real-time)
...
async def startup_event():
    """Warmup the model and pre-cache all voices on startup."""
    ...
    # Pre-cache ALL voices for faster first-request latency
    logging.info("Pre-caching all voices...")
    voice_files = set()
    for f in os.listdir(resources_dir):
        if f.endswith(('.wav', '.mp3', '.flac', '.ogg')):
            voice_name = os.path.splitext(f)[0]
            voice_files.add(voice_name)

    for voice_name in voice_files:
        try:
            # Find the voice file
            matching = [f for f in os.listdir(resources_dir) if f.startswith(voice_name) and f.endswith('.wav')]
            if matching:
                ref_file = f"{resources_dir}/{matching[0]}"
                get_or_create_voice_cache(voice_name, ref_file)
                logging.info(f"Pre-cached voice: {voice_name}")
        except Exception as e:
            logging.warning(f"Failed to pre-cache voice {voice_name}: {e}")

    # Do a warmup generation to compile/optimize
    if voice_files:
        first_voice = list(voice_files)[0]
        logging.info(f"Warmup generation with {first_voice}...")
        try:
            await synthesize_speech("Test.", first_voice)
            logging.info("Warmup complete")
        except Exception as e:
            logging.warning(f"Warmup generation failed: {e}")
